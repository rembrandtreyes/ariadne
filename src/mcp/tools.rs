...
fn string_param_tool(name: &'static str, desc: &'static str, param: &'static str, param_desc: &'static str) -> Tool {
    Tool::new(
        name,
        desc,
        make_schema(
...
impl AriadneService {
    ...
    fn tool_get_call_chain(&self, params: &CallToolRequestParam) -> CallToolResult {
        ...
        match self.with_db(|db| {
            let sym = query::find_symbol_by_name(db, &symbol_name)?
                .ok_or_else(|| anyhow::anyhow!("Symbol not found: {symbol_name}"))?;
            let graph = query::build_call_graph(db)?;
            let mermaid = crate::graph::call_chain::extract_call_chain(&graph, sym.id, false);
            Ok::<_, anyhow::Error>(mermaid)
        }) {
            Ok(Ok(mermaid)) => CallToolResult::success(vec![Content::text(mermaid)]),
            Ok(Err(e)) => CallToolResult::error(vec![Content::text(format!("{e}"))]),
            Err(e) => CallToolResult::error(vec![Content::text(e)]),
        }
    }
    ...
    fn tool_blast_radius(&self, params: &CallToolRequestParam) -> CallToolResult {
        ...
        match self.with_db(|db| {
            let sym = query::find_symbol_by_name(db, &symbol_name)?
                .ok_or_else(|| anyhow::anyhow!("Symbol not found: {symbol_name}"))?;
            let graph = query::build_call_graph(db)?;
            let result = crate::graph::blast_radius::analyze_blast_radius(
                &graph, sym.id as u64, Some(10), false,
            );
    ...
    fn tool_find_dead_code(&self) -> CallToolResult {
        match self.with_db(|db| {
            ...
            let output: Vec<serde_json::Value> = dead
                .iter()
                .map(|s| {
                    let file_path = query::file_path_by_id(db, s.file_id)
    ...
    fn tool_get_complexity(&self) -> CallToolResult {
        match self.with_db(|db| {
            serde_json::json!({
                "files": query::count_files(db).unwrap_or(0),
                "symbols": query::count_symbols(db).unwrap_or(0),
                "calls": query::count_calls(db).unwrap_or(0),
                "resolved_calls": query::count_resolved_calls(db).unwrap_or(0),
                "dead_functions": query::count_dead(db).unwrap_or(0),
                "resolution_rate": query::resolution_rate(db).unwrap_or(0.0),
                "languages": query::get_languages(db).unwrap_or_default(),
            })
        }) {
